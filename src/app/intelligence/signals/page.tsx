    ...
    <div style={{ maxWidth:1100 }}>
      <FadeIn>
        <div style={{ marginBottom:24 }}>
          <div style={S.kicker}>Monitor de Señales</div>
          <h1 style={{ fontFamily:"'Montserrat',sans-serif", fontWeight:700, fontSize:26, letterSpacing:'-0.025em', color:'#2C3539', marginBottom:6 }}>Señales Estratégicas — {signals.length}</h1>
          <p style={{ fontSize:13, color:'#6B7785', lineHeight:1.6 }}>Feed en tiempo real de señales regulatorias, competitivas, demográficas y de mercado. Relevancia ≥70.</p>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.05}>
        <div style={{ ...S.card, marginBottom:20 }}>
          <div style={S.kicker}>Volumen por semana × tipo</div>
      ...
      {actionRequired.length > 0 && (
        <FadeIn delay={0.08}>
          <div style={{ ...S.card, border:'1px solid #E55B4D', marginBottom:20 }}>
            <div style={{ ...S.kicker, color:'#E55B4D' }}>⚡ Acción Requerida — {actionRequired.length}</div>
      ...
      <FadeIn delay={0.1}>
        <div style={{ ...S.kicker, marginBottom:12 }}>Todas las señales</div>
        <StaggerList>
          {signals.map((s:any) => {
                ...
                <div style={{ padding:'13px 15px', borderRadius:8, marginBottom:7, background:s.action_required?'#FFF8F7':'#fff', border:'1px solid #E2E8F0', borderLeft:`4px solid ${s.action_required?'#E55B4D':meta.color}` }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12, marginBottom:5 }}>
                    <div style={{ flex:1 }}>
                      ...
                      {s.action_note && <div style={{ fontSize:12, color:'#E55B4D', fontWeight:600, marginTop:5, padding:'3px 7px', background:'#FBE8E2', borderRadius:4, display:'inline-block' }}>⚡ {s.action_note}</div>}
                    </div>
                  ...
                  <div style={{ display:'flex', gap:5, flexWrap:'wrap', alignItems:'center' }}>
                    <span style={{ fontSize:10, background:meta.bg, color:meta.color, padding:'2px 7px', borderRadius:99, fontWeight:600 }}>{meta.label}</span>
                    {s.entity && <span style={{ fontSize:10, background:'#F5F7FA', color:'#6B7785', padding:'2px 7px', borderRadius:99 }}>{s.entity}</span>}
                    {s.action_required && <span style={{ fontSize:10, background:'#FBE8E2', color:'#E55B4D', padding:'2px 6px', borderRadius:99, fontWeight:700 }}>⚡ Acción</span>}
